...
def startface(frame,cv2image,lmain):
    ...
    cle=cv2.CascadeClassifier("haarcascade_eye.xml")
    gray = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY) 
    faces=clf.detectMultiScale(gray,1.2,6)
    for x,y,w,h in faces:
        cv2.rectangle(cv2image,(x,y),(x+w,y+h),(255,0,0),2)
        face = gray[y:y+h,x:x+w]
        eyes = cle.detectMultiScale(face,1.1,2)
        for ex,ey,ew,eh in eyes:
            cv2.rectangle(cv2image,(x+ex,y+ey),(x+ex+ew,y+ey+eh),(255,0,0),1)
    ...
    while(True):
        ...
        if(flag==True):
            ...
            for x,y,w,h in faces:
                ...
                face = gray[y:y + h, x:x + w]
                eyes = cle.detectMultiScale(face, 1.1, 8)
                for ex, ey, ew, eh in eyes:
...
def show_frame():
    ...
    for x,y,w,h in faces:
        if(flag==True):
            cv2.rectangle(cv2image,(x,y),(x+w,y+h),(255,0,0),2)
            face = gray[y:y + h, x:x + w]
            eyes = cle.detectMultiScale(face, 1.1, 6)
            for ex, ey, ew, eh in eyes:
                cv2.rectangle(cv2image, (x + ex, y + ey), (x + ex + ew, y + ey + eh), (255, 0, 0), 1)
    img = Image.fromarray(cv2image)
    imgtk = ImageTk.PhotoImage(image=img)
